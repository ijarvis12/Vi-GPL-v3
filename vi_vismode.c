...
gvoid visualmode_main(gint visual_command) {
  ...
  switch(visual_command) {
    ...
    case 'r':
      ...
      if(visual_command > 126) break;
      else if(visual_command < 32 && visual_command != 10) break; /* Includes escape (27), excludes newline (10) */
      visualmode_main('x');
    ...
    case KEY_DOWN:
      ...
      i=0;
      do {
        if((gbuffer[g].gtop_line + gbuffer[g].ypos) == gbuffer[g].gtotal_lines) {
          break;
        }
    ...
    if(getline(&line, &len, gbuffer[g].gtemp_files) > 0) {
      ...
      while((l+incr_l) < strlen(line) && y<maxy) {
        do {
          ...
          /* Reseek file position (for later) */
          if(y == gbuffer[g].ypos && l == gbuffer[g].xpos)
            gtemp_pos = ftell(gbuffer[g].gtemp_files) + l + incr_l;
          l++;
